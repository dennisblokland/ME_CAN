...
class SensorData {
	public:
		SensorData(String unit, String name = ""){
			Unit = unit;
			Name = name;
		}
		SensorData(){
...
{
	public:
		SensorData<uint16_t> RPM = SensorData<uint16_t>("", "Engine RPM"); /* Engine RPM, units none  */
		SensorData<float_t> TPS = SensorData<float_t>("%", "TPS"); /* Throttle position sensor, units %  */
		SensorData<float_t> MAP = SensorData<float_t>("kPa", "MAP"); /* Manifold air pressure, units kPa  */
		SensorData<float_t> IAT = SensorData<float_t>("°C" , "IAT"); /* Intake air temperature, units deg C  */
		SensorData<float_t> RPM_HardLimit = SensorData<float_t>("RPM");/* Engine RPM limit, units none  */
		SensorData<float_t> Lambda_Trim; /* Lambda Trim, units none  */
		SensorData<float_t> AFRCurr_1 = SensorData<float_t>("", "AFR 1"); /* Current Air Fuel ratio 1, units none  */
		SensorData<float_t> AFRCurr_2 = SensorData<float_t>("", "AFR 2"); /* Current Air Fuel ratio 2, units none  */
		SensorData<float_t> AFR_Target = SensorData<float_t>("", "AFR Target"); /* Air Fuel target, units none  */
		...
		SensorData<float_t> Oil_TEMP = SensorData<float_t>("°C", "Oil Temp"); /* Oil temperature, units deg C  */
		SensorData<float_t> Oil_PRESS = SensorData<float_t>("kPa", "Oil Pressure"); /* Oil pressure, units kPa  */
		SensorData<float_t> COOL_TEMP = SensorData<float_t>("°C", "CLT"); /* Coolant temperature, units deg C  */
		SensorData<float_t> VBAT = SensorData<float_t>("V", "VBAT");   /* Battery coltage, units V  */
		SensorData<float_t> MAP_Target= SensorData<float_t>("kPa"); /* MAP target pressure, units kPa  */
		SensorData<float_t> Vehicle_Speed = SensorData<float_t>("km/h", "Speed"); /* Vehicle speed, units kph  */
		SensorData<uint16_t> EPS_Ev_Msk; /* Protection module was triggered, units none  */
		SensorData<uint16_t> Gear_Pos; /* Current gear position, units none  */
		...
		SensorData<uint16_t> Vehicle_Speed_2 = SensorData<uint16_t>("km/h", "Speed");  /* Vehicle speed 2, units kph  */
	private:
